...
function injectAllShelves(e) {
    ...
    }).fail(createErrorHandler("Retrieve All Shelves for Product")).always(function () {
    });
    ...
    $(selector).popover({
        ...
        if (data) {
            ...
            data.product_attributes.all_shelves.values.forEach(function (path) {
                if (path.path_id !== primaryShelfPathId) {
                    ...
                    (function (ds) {
                        $(ds).on("click", toggleItemPagePopover)
                    })(dataSelector)
                }
            })
        }

        $(".js-all-shelves-popup").find(".js-close-btn").off().on("click", onTogglePopover);
        ...
        $(selector2).popover({
            ...
            $(selector2).parent().find(".js-select-tag_").select2(createSelectOptions()).on("change", function (e) {
                $(selector2).parent().find(".js-tag-path_").text("(" + getItemPath(e.added) + ")");
            ...
            $(selector2).parent().off("click", ".js-cancel-btn").on("click", ".js-cancel-btn", onTogglePopover);

        }).on("hide.bs.popover", function (e) {
            $(selector2).find(".js-select-tag_").select2("destroy");
        });

        $(selector2).on("click", toggleItemPagePopover)

    }).on("hide.bs.popover", function (e) {
        $(".js-all-shelves-popup").find(".js-select-tag_").select2("destroy");
    });

    $(selector).on("click", toggleItemPagePopover);


}

function toggleItemPagePopover(e) {
...
function onConfirmReject() {
    ...
    var valueIds = shelf ? [shelf.id] : [null];
    var shelfName = shelf ? shelf.name : null;
...
function onConfirmAdd(e) {
    ...
    }).fail(createErrorHandler("Add New Shelf for Item")).always(function () {
        spinner.stop();
        $popover.popover("hide");
    });
